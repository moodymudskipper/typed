...
functions), thus `Integer(3)` and
[`Character()`](https://moodymudskipper.github.io/typed/reference/assertion_factories.md)
are functions.
...
We call `Integer(3)` and
[`Character()`](https://moodymudskipper.github.io/typed/reference/assertion_factories.md)
assertions, and we call `Integer` and `Character` assertion factories
(or just types, with then we must be careful not to confuse them with
atomic types returned by the `typeof` function).

The package contains many assertion factories (see
[`?assertion_factories`](https://moodymudskipper.github.io/typed/reference/assertion_factories.md)),
the main ones are:
